...
public class Address extends javax.swing.JDialog {
    ...
    public Address(Frame parent, String tableName1, Integer nic1, String name, String title, boolean modal) {
        super(parent, modal);
        initComponents();
        nic = nic1;
        ...
        HashMap m1 = new Onload().getComboData("distric");
...
//        jComboBox2.setModel((DefaultComboBoxModel) new Onload().getComboData("city").get(1));
    ...
    private void initComponents() {
        ...
        setUndecorated(true);
        setPreferredSize(new java.awt.Dimension(612, 486));
        setResizable(false);
...
//                JOptionPane.showMessageDialog(this, rs.getString("city.name"));
    ...
    private ResultSet getUserData() throws Exception {
        ...
        ResultSet rs = MySql.select("SELECT * FROM `" + tableName + "` INNER JOIN `address` ON `address`.`id` = `" + tableName + "`.`address_id` INNER JOIN `city` ON `city`.`id` = `address`.`city_id` "
                + "INNER JOIN `distric` ON `city`.`district_id` = `distric`.`id` WHERE `" + tableName + "`.`id` = '" + nic + "';");
